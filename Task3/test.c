...
//          override_free_memory(3, *name_rev_folder, *path_rev_folder, *path_origin_folder);
...
//    if(path_new_rev_folder == NULL) {
//         closedir(dir);
//         override_free_memory(1, name_rev_folder);
...
//    if(path_new_origin_folder == NULL) {
//         closedir(dir);
//         override_free_memory(2, name_rev_folder, path_new_rev_folder);
...
//            override_free_memory(3, name_rev_folder, path_new_rev_folder, path_new_origin_folder);
//            closedir(dir);
//            return ERROR;
//        }
//
...
/// \param path_reverse_folder
/// \return  STATUS: ERROR/OK
ssize_t create_reverse_folder(const char* path_origin_folder, const char* path_reverse_folder) {
    int ret_mkdir = mkdir(path_reverse_folder, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    if(ret_mkdir == ERROR) {
...
ssize_t parse_command_line(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (create_reverse_folder(argv[i], path_to_folder) == ERROR) {
            override_free_memory(2, path_to_folder, rev_folder);
            return ERROR;
        }
    }
